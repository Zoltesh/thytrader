...
Paper may start on closed **1h or 5m** bars of a published strategy. Live may start on closed **1h or 5m** bars. 5m live pauses unless the user-order feed is connected. Do not start paper or live on a published strategy that declares `htf_filter`; the API rejects those deployments.
