...
The `products` payload lists enabled USD spot products. The `indicators` payload lists implemented kinds only: ema, sma, rsi, atr, volume_sma, highest, lowest, stdev, roc, williams_r, cci. Highest is locked to high, lowest to low, stdev and roc to close. Williams %R and CCI lock high/low/close like ATR. MACD, Bollinger, and other kinds are not present.
