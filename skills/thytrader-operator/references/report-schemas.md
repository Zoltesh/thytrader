...
The `products` payload lists enabled USD spot products. The `indicators` payload lists implemented kinds only: ema, sma, rsi, atr, volume_sma, highest, lowest, stdev, roc, williams_r, cci, identity, constant. Highest is locked to high, lowest to low, stdev and roc to close. Williams %R and CCI lock high/low/close like ATR. Identity selects one of open/high/low/close/volume. Constant omits input and declares value. Each row includes `parameter_kind` (`period`, `none`, or `value`). MACD, Bollinger, and other kinds are not present.
