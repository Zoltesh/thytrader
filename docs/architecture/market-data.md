...
The worker maintains immutable, fingerprint-addressed 1h, 5m, 15m, and 30m historical datasets. Initial
...
| Timeframe | `1h`, `5m`, `15m`, and `30m` for complete-only datasets; research and **paper** remain `1h` and `5m`; live remains `1h` |
...
1. **Additional timeframes** — 5m research datasets and 15m/30m complete-only datasets (Phase 7
   slices 1–2) are implemented; 6h and 1d remain planned Phase 7 slices. Strategy, paper, and live
   clocks do not accept `15m` or `30m`.
2. **Additional ingestion targets** — an explicit watchlist plus confirmation-gated `thytrader-data` ingest cover extra USD spot products, 5m, 15m, and 30m without weakening complete-only publication.
