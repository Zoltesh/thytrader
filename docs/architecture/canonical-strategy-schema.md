...
- 1h Coinbase USD spot for live; `1h` or `5m` for research, backtests, and paper; long only, one position, with EMA/SMA/RSI/ATR/volume-SMA/`highest`/`lowest`/`stdev`/`roc`/`williams_r`/`cci`/`identity`/`constant` indicators;
...
- Single-source `input` must be one of: `open`, `high`, `low`, `close`, `volume`. Each shipped kind
  locks that field: EMA/SMA/RSI/`stdev`/`roc` use `close`; `highest` uses `high`; `lowest` uses `low`;
  `volume_sma` uses `volume`. ATR, `williams_r`, and `cci` use the canonical ordered array
  `["high", "low", "close"]`. `identity` selects exactly one of those single-source fields.
