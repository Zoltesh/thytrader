...
2. calculates the canonical catalog (EMA, SMA, RSI, ATR, volume SMA, plus the ADR 0026
   ...
   `identity`/`constant` kinds, the ADR 0029 `wma`/`momentum`/`mfi` kinds, the ADR 0032
   `macd`/`bollinger` kinds, and the ADR 0047 `stochastic`/`adx`/`stdev_sample` kinds with
   configurable rolling inputs) sequentially under the isolated
