...
The benchmark response carries the source identities, execution assumptions, entry/exit evidence, costs, net return, maximum drawdown, and a `benchmark_fingerprint`. That SHA-256 identity covers the complete derived evidence payload except the identity field itself; the API revalidates and checks it before response serialization so a dependency-supplied model copy cannot replace one derived value while retaining a valid shape. The report is not persisted as part of canonical result JSON and cannot mutate or re-fingerprint an existing result. If the result, source run, or dataset cannot be reverified, the endpoint fails closed with a redacted `503` response.
