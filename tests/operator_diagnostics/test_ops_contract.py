...
def test_ops_contract_matches_requires_payload() -> None:
    ...
    assert expected["id"] == "thytrader-ops-contract-v11"
    assert expected["expected_schema_revision"] == EXPECTED_SCHEMA_REVISION
    assert expected["expected_schema_revision"] == "0025"
    ...
    assert expected["paper_timeframes"] == ["1h", "5m"]
    assert expected["live_timeframes"] == ["1h", "5m"]
