...
def test_ingest_once_publishes_complete_five_minute_range(tmp_path: Path) -> None:
    """Five-minute ingest uses get_range and writes under the 5m partition."""

    class _FiveMinuteService:
        ...
        async def get_range(
            ...
            """Return a complete 5m range for the requested window."""
            self.requests.append((product_id, timeframe, starts_at, ends_at))
            count = int((ends_at - starts_at) / timeframe.duration)
            candles = tuple(
                Candle(
    ...
    async def exercise() -> None:
        ends_at = datetime(2026, 7, 29, 2, 15, tzinfo=UTC)
        service = _FiveMinuteService()
        state_store = InMemoryMarketDataWorkerStateStore()
        await ingest_once(
            service=service,
            dataset_store=DatasetStore(tmp_path),
        ...
        assert state.complete is True
        assert state.status is MarketDataWorkerStatus.SUCCEEDED
        assert service.requests[0][1] is CandleInterval.FIVE_MINUTES
        manifests = tuple((tmp_path / "manifests").glob("*.json"))
        assert len(manifests) == 1
        assert "5m" in manifests[0].read_text()

    asyncio.run(exercise())


def test_ingest_once_publishes_complete_fifteen_minute_range(tmp_path: Path) -> None:
    """Fifteen-minute ingest uses get_range and writes under the 15m partition."""

    class _FifteenMinuteService:
        """Stub that only implements the multi-interval range boundary."""
        ...
        async def get_range(
            ...
            """Return a complete 15m range for the requested window."""
            self.requests.append((product_id, timeframe, starts_at, ends_at))
            count = int((ends_at - starts_at) / timeframe.duration)
            candles = tuple(
                Candle(
    ...
    async def exercise() -> None:
        ...
        assert state.complete is True
        assert state.status is MarketDataWorkerStatus.SUCCEEDED
        assert service.requests[0][1] is CandleInterval.FIFTEEN_MINUTES
        manifests = tuple((tmp_path / "manifests").glob("*.json"))
        assert len(manifests) == 1
        assert "15m" in manifests[0].read_text()
...
def test_ingest_once_publishes_complete_six_hour_range(tmp_path: Path) -> None:
    """Six-hour ingest uses get_range and writes under the 6h partition."""

    class _SixHourService:
        """Stub that only implements the multi-interval range boundary."""
        ...
        async def get_range(
            ...
            """Return a complete 6h range for the requested window."""
            self.requests.append((product_id, timeframe, starts_at, ends_at))
            count = int((ends_at - starts_at) / timeframe.duration)
            candles = tuple(
                Candle(
    ...
    async def exercise() -> None:
        ...
        assert state.complete is True
        assert state.status is MarketDataWorkerStatus.SUCCEEDED
        assert service.requests[0][1] is CandleInterval.SIX_HOURS
        manifests = tuple((tmp_path / "manifests").glob("*.json"))
        assert len(manifests) == 1
        assert "6h" in manifests[0].read_text()
        verified = DatasetStore(tmp_path).load_candles(state.content_fingerprint or "")
        assert verified[0].starts_at == ends_at - timedelta(hours=6)

    asyncio.run(exercise())


def test_ingest_once_publishes_complete_one_day_range(tmp_path: Path) -> None:
    """Daily ingest uses get_range and writes under the 1d partition."""

    class _OneDayService:
        """Stub that only implements the multi-interval range boundary."""
        ...
        async def get_range(
            ...
            """Return a complete 1d range for the requested window."""
            self.requests.append((product_id, timeframe, starts_at, ends_at))
            count = int((ends_at - starts_at) / timeframe.duration)
            candles = tuple(
                Candle(
    ...
    async def exercise() -> None:
        ...
        assert state.complete is True
        assert state.status is MarketDataWorkerStatus.SUCCEEDED
        assert service.requests[0][1] is CandleInterval.ONE_DAY
        manifests = tuple((tmp_path / "manifests").glob("*.json"))
        assert len(manifests) == 1
        assert "1d" in manifests[0].read_text()
        verified = DatasetStore(tmp_path).load_candles(state.content_fingerprint or "")
        assert verified[0].starts_at == ends_at - timedelta(days=1)
