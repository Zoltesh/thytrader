...
while `/health/ready` is 200. Matching `0.1.0` alone is not evidence that the image is current. Read
`uv run thytrader-operator configuration` → `payload.ops_contract` for the expected Alembic revision
instead of hard-coding a migration number.
